...
const size_t   CRYPTONOTE_MINED_MONEY_UNLOCK_WINDOW		= 1; // 20m unlock
const uint64_t CRYPTONOTE_BLOCK_FUTURE_TIME_LIMIT		= 1 * 1 * 2; // was 2 hours, not 3 * T
const uint64_t CRYPTONOTE_BLOCK_FUTURE_TIME_LIMIT_V1	= 30; // LWMA3
const uint64_t CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE		= 1; // 20m unlock

const size_t   BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW		= 3;
const size_t   BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW_V1		= 1; // LWMA3
...
const uint64_t DIFFICULTY_TARGET				= 12; // seconds = 2.20m
const uint64_t EXPECTED_NUMBER_OF_BLOCKS_PER_DAY		= 2 * 6 * 6 / DIFFICULTY_TARGET;
...
const uint32_t DEPOSIT_MAX_TERM					= 1 * 1 * 21900; // ~1 year
const uint32_t DEPOSIT_MAX_TERM_V1					= 6 * 2; // ~5 year
...
const uint64_t MAX_BLOCK_SIZE_GROWTH_SPEED_NUMERATOR		= 1 * 10;
const uint64_t MAX_BLOCK_SIZE_GROWTH_SPEED_DENOMINATOR		= 3 * 2 * 6 * 6 / DIFFICULTY_TARGET;
...
const uint64_t CRYPTONOTE_MEMPOOL_TX_LIVETIME			= (6 * 6 * 2); // seconds, 12 hours
const uint64_t CRYPTONOTE_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME	= (6 * 6 * 4); // seconds, 1 day
